...
const DordoyBanner = () => {
  const [hasVisited, setHasVisited] = useState(
    typeof window !== 'undefined' ? localStorage.getItem('visited') === 'true' : false
  );
  const [showIntro, setShowIntro] = useState(!hasVisited);
  const [currentSlide, setCurrentSlide] = useState(0);
  const controls = useAnimation();
  const containerRef = useRef(null);

  // Данные для слайдов с современным дизайном
  const slides = [
    {
      title: "АЛГА ДОРДОЙ",
      subtitle: "Непобедимый дух Кыргызстана",
      media: "/videos/dordoy-hero.mp4",
      cta: "Купить билеты",
      color: "#FFD700",
    ...
    },
    {
      ...
      media: "/videos/dordoy-history.mp4",
      cta: "История клуба",
      color: "#005BBB",
  ...
  // Анимация текста с эффектом "волны"
        ...
        {showIntro && (
          ...
          >
            <div className="relative text-center">
              {/* Эффект "вспышки" */}
              <motion.div
                className="absolute inset-0 bg-yellow-400 rounded-full mix-blend-color-dodge"
              ...
              >
                Официальный клуб - Непобедимый дух Кыргызстана
              </motion.p>
            </div>
          </motion.div>
        )}
            ...
            >
              {slides[currentSlide].stats.map((stat, i) => (
                <div key={i} className="text-center">
                  ...
                  <div className="text-sm uppercase tracking-wider text-white/80 mt-1">
                    {stat.label}
                  </div>
                </div>
              ))}
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-4">
              {slides.map((_, i) => (
                ...
                >
                  <div 
                    className={`w-8 h-1 rounded-full transition-all ${currentSlide === i ? 'bg-white' : 'bg-white/30'}`}
                  >
                    {currentSlide === i && (
                      <motion.div
